                        ...
                        <form method="POST" action="{{ route('payment.update', $payment->id) }}">
                                ...
                                <select name="pegawai_id" id="pegawai_id" class="form-control select2">
                                    ...
                                    @foreach ($pegawaiList as $pegawai)
                                        <option value="{{ $pegawai->id }}"
                                            {{ $pegawai->id == $payment->pegawai_id ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="total_tugas_honor" id="total_tugas_honor" readonly
                                    class="form-control" value="{{ $payment->tugas_honor }}" autocomplete="off">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="koperasi" id="koperasi" class="form-control"
                                    value="{{ old('koperasi', $payment->koperasi) }}">
                            </div>
                                ...
                                <select name="channel_id" id="channel_id" class="form-control">
                                    @foreach ($channelList as $channel)
                                        <option value="{{ $channel->id }}"
                                            {{ $channel->id == $payment->channel_id ? 'selected' : '' }}>
                                            {{ $channel->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
